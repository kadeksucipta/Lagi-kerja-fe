...
const Software = () => {
  ...
  const fetchSoftware = () => {
    fetch(`https://lagi-kerja-production.up.railway.app/api/software`)
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    <React.Fragment>
      ...
      {software.map((item, index) => (
        <div key={index}>
          <img
            className="detail-softwareimg"
            src={"https://lagi-kerja-production.up.railway.app/images/" + item.image_url}
          />
          <div style={{margin: "20px"}}>
          <h1 className="title-software">{item.name}</h1>
          <h3>{item.jenis}</h3>
          <h6><FontAwesomeIcon icon={faCircleDot} /> {item.category.name}</h6>
          <h5>
            {" "}
          ...
          <h6>Sallary</h6>
          <p><strong>Rp.{numberWithCommas(item.price)}</strong></p>
          <Button className="btn-detailsoftware">Lamar sekarang</Button>
          </div>
        </div>
      ))}
      <Footer />
